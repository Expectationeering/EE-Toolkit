...
4. **Report completion** with exactly these four lines: (a) uv `<version>` and pandoc `<version>` installed or already present; (b) environment created at `.venv` by `uv sync`; (c) verification: `import docx, lxml` → ok; (d) next step: no terminal restart needed — run a flow now, e.g. `Execute flows/ee-flow`.
