...
name: execute-flow
description: >-
  Orchestrate an EE-Toolkit flow end to end: ingest inputs, drive the specialist
  agents step by step per flows/<name>/flow.md, assemble the live workbook,
  run the QA audit loop, convert to .docx, and write run statistics. Use when
  the user asks to execute/run/start a flow under flows/ — e.g. "Execute
  flows/expectationeering-flow", "start expectationeering", "run
  expectationeering", or just the flow name.
---
